# 06_playbooks_recovery – Playbooks de recuperación (P01–P04)

Este directorio contiene los **playbooks de recuperación** de ORKO v1.0.0, diseñados para actuar cuando la organización se encuentra en estados de salud comprometidos o con riesgo de degradación:

- `P01_low_h_org_recovery` – Recuperación de H_org cuando cae a bandas críticas o de riesgo.
- `P02_handoff_reduction` – Reducción de handoffs excesivos entre equipos/sistemas.
- `P03_okr_alignment` – Alineamiento de objetivos (OKR) cuando hay desajuste entre niveles.
- `P04_security_remediation` – Remediación de incidentes de seguridad que afectan la estabilidad organizacional.

En el diseño de gates y trayectorias:

- Estos playbooks se activan principalmente bajo **G1_H_org_Critico** y **G2_H_org_Bajo_Riesgo** (`02_health_gates.md`).
- Se usan en combinación con las trayectorias `Survival` y `Minimal` (`09_trayectorias/04_survival_0_10K.md`, `09_trayectorias/01_minimal_6_12_meses.md`) para estabilizar H_org y permitir volver a estados más sanos.
- La selección y activación se describe en la `03_decision_matrix.md` (escenarios DM1/DM2/DM5) y en las fases F13/F14/F17.

Relación con VG3 y VG4:

- VG3 (CAP-16) establece P01–P15 como **contrato estable** de playbooks; este README solo documenta el rol de P01–P04 y no modifica `playbook_schema.yaml` ni `playbook_instances.yaml`.
- En VG4 (CAP-17/CAP-19), los casos de `40_implementacion_metodologia/ejemplos/*_completo/` usan P01–P04 como parte de las estrategias de recuperación y ajuste de trayectoria, especialmente en los contextos `startup_50p`, `scaleup_200p` y `gore_nuble`.

Para detalles completos de cada playbook, ver los archivos `P01_*.md`–`P04_*.md` de este directorio y las referencias en:

- `30_metodologia_orko/13_metricas_validacion/02_health_gates.md`
- `30_metodologia_orko/09_trayectorias/03_decision_matrix.md`
- `30_metodologia_orko/17_validacion_final/validation_final_report.md` (§1.3, §2.5)
